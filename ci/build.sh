...
CMAKE_OPTIONS="${CMAKE_OPTIONS} -DREPOSITORY_LOC=${PREFIX}/etc-sysrepo -DDAEMON_PID_FILE=${RUN_TMP}/sysrepod.pid -DDAEMON_SOCKET=${RUN_TMP}/sysrepod.sock -DPLUGIN_DAEMON_PID_FILE=${RUN_TMP}/sysrepo-plugind.pid -DSUBSCRIPTIONS_SOCKET_DIR=${RUN_TMP}/sysrepo-subscriptions -DGEN_LANGUAGE_BINDINGS=ON -DGEN_PYTHON_BINDINGS=OFF" emerge_dep sysrepo
...
CMAKE_OPTIONS="${CMAKE_OPTIONS} -DPIDFILE_PREFIX=${RUN_TMP}" emerge_dep Netopeer2
do_test_dep_cmake Netopeer2 --timeout 30 -j${CI_PARALLEL_JOBS}
